...
class CenterNetPoseOrient(SingleStageDetector):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(CenterNetPoseOrient, self).__init__(backbone, neck, bbox_head, train_cfg,
    ...
    def forward(self, img, img_metas, return_loss=True, pose_result=False, **kwargs):
        ...
        if return_loss:
            ...
            if pose_result:
                return self.forward_test_pose_orient(img, img_metas, **kwargs)
            else:
                return self.forward_test(img, img_metas, **kwargs)

    def forward_test_pose_orient(self, imgs, img_metas, **kwargs):
        """
        Args:
            imgs (List[Tensor]): the outer list indicates test-time
                augmentations and inner Tensor should have a shape NxCxHxW,
    ...
    def simple_test_pose(self, img, img_metas, rescale=False):
        ...
        """
        feat = self.extract_feat(img)
        results_list = self.bbox_head.simple_test_pose_orient(
            feat, img_metas, rescale=rescale)
        # bbox_results = [
        #     bbox2result(det_bboxes, det_labels, self.bbox_head.num_classes)
