...
class CenterNetPoseOrientHead(BaseDenseHead, BBoxTestMixin):
    ...
    def __init__(self,
                 ...
                 num_keypoints=17,
                 # orients
                 loss_orients=dict(type='L1Loss', loss_weight=1.0),
                 loss_orients_heatmap=dict(type='GaussianFocalLoss', loss_weight=1.0)):

        super(CenterNetPoseOrientHead, self).__init__(init_cfg)
        ...
        # orients
        self.orients_head = self._build_head(in_channel, feat_channel, 2)
        self.orients_heatmap_head = self._build_head(in_channel, feat_channel, 1)

        self.loss_orients = build_loss(loss_orients)
        self.loss_orients_heatmap = build_loss(loss_orients_heatmap)
    ...
    def init_weights(self):
        ...
        orients_bias_init = bias_init_with_prob(0.1)
        self.orients_heatmap_head[-1].bias.data.fill_(orients_bias_init)

        for head in [self.wh_head, self.offset_head, self.keypoint_wh_head,
                     self.keypoint_offset_head, self.orients_head]:
    ...
    def forward_single(self, feat):
        ...
        # orients
        # 将模型的预测输出范围限制到[-1, 1]
        orients_pred = self.orients_head(feat).sigmoid() * 2 - 1
        orients_heatmap_pred = self.orients_heatmap_head(feat).sigmoid()

        return center_heatmap_pred, wh_pred, offset_pred, keypoint_heatmap_pred,\
               keypoint_wh_pred, keypoint_offset_pred, orients_pred, orients_heatmap_pred
    ...
    def loss(self,
        ...
        # orients
        orient_heatmap_pred = orients_heatmap_pred[0]
        orient_pred = orients_pred[0]
        ...
        target_result, avg_factor = self.get_pose_orient_targets(gt_bboxes, gt_keypoints, gt_labels,
                                                                 gt_orients, gt_orients_mask,
                                                                 center_heatmap_pred.shape,
                                                                 img_metas[0]['pad_shape'])
        ...
        # loss orients
        loss_orients_heatmap = self.loss_orients_heatmap(orient_heatmap_pred,
                                                         orients_center_heatmap_target,
                                                         avg_factor=avg_factor)
        loss_orients = self.loss_orients(
                                orient_pred,
                                orients_target,
                                orients_target_weight,
                                avg_factor=avg_factor * 2)
        ...
        return dict(
            ...
            loss_orients_heatmap=loss_orients_heatmap,
            loss_orients=loss_orients,
        )
    ...
    def get_pose_orient_targets(self, gt_bboxes, gt_keypoints, gt_labels,
                                gt_orients, gt_orients_mask, feat_shape, img_shape):
        """Compute regression and classification targets in multiple images.
        ...
        # orients
        orients_center_heatmap_target = gt_bboxes[-1].new_zeros(
            [bs, 1, feat_h, feat_w])
        orients_target = gt_bboxes[-1].new_zeros([bs, 2, feat_h, feat_w])
        orients_target_weight = gt_bboxes[-1].new_zeros([bs, 2, feat_h, feat_w])
        ...
        for batch_id in range(bs):
            ...
            gt_keypoint = gt_keypoints[batch_id]
            gt_orient = gt_orients[batch_id]
            gt_orient_mask = gt_orients_mask[batch_id]

            assert gt_bbox.shape[0] == gt_keypoint.shape[0], "gt_bbox[0] == gt_keypoint.shape[0]"
            assert gt_bbox.shape[0] == gt_orient.shape[0], "gt_bbox[0] == gt_orient.shape[0]"

            for j, (ct, _gt_keypoint, _gt_orient, _gt_orient_mask) in \
                    enumerate(zip(gt_centers, gt_keypoint, gt_orient, gt_orient_mask)):
                ctx_int, cty_int = ct.int()
                if 0 <= ctx_int < feat_w and 0 <= cty_int < feat_h:
                    ctx, cty = ct
                    scale_box_h = (gt_bbox[j][3] - gt_bbox[j][1]) * height_ratio
                    ...
                    # orients
                    if _gt_orient_mask:
                        gen_gaussian_target(orients_center_heatmap_target[batch_id, 0],
                                            [ctx_int, cty_int], radius)
                        # 将MEBOW的格式转为[-pi, pi], 将角度的范围转换为kitti数据的格式
                        if _gt_orient > 180:
                            _gt_orient = _gt_orient - 360

                        _gt_orient = -_gt_orient
                        _gt_orient = _gt_orient - 90
                        if _gt_orient > 180:
                            _gt_orient = _gt_orient - 360
                        if _gt_orient < -180:
                            _gt_orient = _gt_orient + 360

                        _gt_orient = _gt_orient / 180 * math.pi

                        orients_target[batch_id, 0, cty_int, ctx_int] = math.sin(_gt_orient)
                        orients_target[batch_id, 1, cty_int, ctx_int] = math.cos(_gt_orient)
                        orients_target_weight[batch_id, :, cty_int, ctx_int] = 1
        ...
        target_result = dict(
            ...
            # orients
            orients_center_heatmap_target=orients_center_heatmap_target,
            orients_target=orients_target,
            orients_target_weight=orients_target_weight,
        )
        return target_result, avg_factor
    ...
    def forward_train(self,
        ...
        outs = self(x)
        # outs的列表如下:
        # center_heatmap_pred, wh_pred, offset_pred, keypoint_heatmap_pred, \
        # keypoint_wh_pred, keypoint_offset_pred, orients_pred, orients_heatmap_pred
        ...
        assert gt_labels is not None, "gt_labels should not None"
        assert gt_keypoints is not None, "gt_keypoints should not None"
        loss_inputs = outs + (gt_bboxes, gt_labels, gt_keypoints, gt_orients, gt_orients_mask, img_metas)
    ...
    def simple_test_pose_orient(self, feats, img_metas, rescale=False):
        outs = self.forward(feats)
        results_list = self.get_bboxes_pose_orient(
            *outs, img_metas=img_metas, rescale=rescale)
        return results_list

    def get_bboxes_pose_orient(self,
                   center_heatmap_preds,
                   wh_preds,
                   offset_preds,
                   keypoint_heatmap_pred,
        ...
        for img_id in range(len(img_metas)):
            result_list.append(
                self._get_bboxes_pose_orient_single(
                    center_heatmap_preds[0][img_id:img_id + 1, ...],
                    wh_preds[0][img_id:img_id + 1, ...],
                    offset_preds[0][img_id:img_id + 1, ...],
                    ...
                    orients_pred[0][img_id:img_id + 1, ...],
                    orients_heatmap_pred[0][img_id:img_id + 1, ...],
                    img_metas[img_id],
                    rescale=rescale,
                    with_nms=with_nms))
        return result_list

    def _get_bboxes_pose_orient_single(self,
                           center_heatmap_pred,
                           wh_pred,
                           offset_pred,
                           keypoint_heatmap_pred,
        ...
        Returns:
            tuple[Tensor, Tensor]: The first item is an (n, 5) tensor, where
                ...
                corresponding box.
        """
        batch_det_bboxes, batch_labels, batch_keypoint_wh, \
        batch_orients_score, batch_orients = self.decode_heatmap_pose_orient(
            center_heatmap_pred,
            wh_pred,
            offset_pred,
            keypoint_heatmap_pred,
        ...
        orients_score = batch_orients_score.view([-1, 1])
        orients = batch_orients.view([-1, 1])
        ...
        #     det_bboxes, det_labels = self._bboxes_nms(det_bboxes, det_labels,
        #                                               self.test_cfg)
        return det_bboxes, det_labels, keypoint_wh, orients_score, orients

    def decode_heatmap_pose_orient(self,
                       center_heatmap_pred,
                       wh_pred,
                       offset_pred,
                       keypoint_heatmap_pred,
        ...
        *batch_dets, topk_ys, topk_xs = get_topk_from_heatmap(
        ...
        if 1:
            ...
            orients = torch.arctan(orients[:, :, 0]/orients[:, :, 1]) / pi * 180

            batch_orients_score = orients_score
            batch_orients = orients

        return batch_bboxes, batch_topk_labels, batch_keypoint_wh, batch_orients_score, batch_orients
